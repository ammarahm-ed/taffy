...
pub unsafe extern "C" fn Java_app_visly_stretch_Style_nFree(_: JNIEnv, _: JObject, style: jlong) {
    let _style = Box::from_raw(style as *mut FlexboxLayout);
}
...
pub unsafe extern "C" fn Java_app_visly_stretch_Node_nConstruct(
    ...
    let style = Box::from_raw(style as *mut FlexboxLayout);
    let node = stretch.new_with_children(*style, &children).unwrap();
...
pub unsafe extern "C" fn Java_app_visly_stretch_Node_nConstructLeaf(
    ...
    let node = stretch
        .new_leaf_with_measure(
            *style,
            taffy::node::MeasureFunc::Boxed(Box::new(move |constraint| {
                ...
                let result = env.call_method(
                    ...
                    "(FF)[F",
                    &[
                        JValue::from(constraint.width.unwrap_or(f32::NAN)),
                        JValue::from(constraint.height.unwrap_or(f32::NAN)),
                    ],
                );
                ...
                match result {
                    ...
                    Err(_) => constraint.map(|v| v.unwrap_or(0.0)),
                }
            })),
        )
        .unwrap();
...
pub unsafe extern "C" fn Java_app_visly_stretch_Node_nSetMeasure(
    ...
    stretch
        .set_measure(
            *node,
            Some(taffy::node::MeasureFunc::Boxed(Box::new(move |constraint| {
                let env = jvm.attach_current_thread().unwrap();
                let result = env.call_method(
                    measure.as_obj(),
                    "measure",
